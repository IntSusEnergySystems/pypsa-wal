...
### Host health — the workstation, not the model

Long runs on this machine have twice been stopped by the **desktop session**,
not by anything in the workflow: a wedged COSMIC portal filled `/` with a 54 GB
syslog, and `gcr-ssh-agent` spun at 77 % so every `ssh` to NIC5 hung. Both are
documented, with the fixes and the one-time hardening, in
[`run_from_home_computer.md`](run_from_home_computer.md).

Before any long run, and from the monitoring loop:

```bash
./scripts/walloon_scripts/check_host_health.sh          # report
./scripts/walloon_scripts/check_host_health.sh --fix    # report and self-heal
```

It checks free space on **`/` as well as `/home`** — it is always the 95 GB root
partition that fills, never the 3.5 TB `/home` where results live — plus the rate
syslog is growing, the two daemons that wedge, and that the active VPN really is
`sqvpn`. Exit 0 healthy, 1 warning, 2 act now.
...
```
results/walloon/
├── index.html                          ← cross-scenario site entry (pypsa2html)
├── scen_demande_haute/
│   ├── csvs/  graphs/  maps/  networks/  explorer/  logs/
│   └── html/
│       ├── index.html                  ← pypsa-wal hub (links the two reports)
│       ├── pypsa/                      ← pypsa2html (library, unmodified)
│       │   ├── index.html
...
pypsa2html is a general library; we do not add Walloon-specific links to it.
TIMES pages therefore live in `html/times/`, and `html/index.html` is a tiny
hub this repo owns. The scenario switcher uses relative links inside `pypsa/`.
...
[`rules/pypsa2html.smk`](rules/pypsa2html.smk) is included from the `Snakefile`.
If `pypsa2html` is importable, `rule all` builds
`results/walloon/<scenario>/html/pypsa/index.html`; if it is not, the rule is never
defined and the workflow is unaffected. The project config is
[`config/pypsa2html.yaml`](config/pypsa2html.yaml) (Walloon paths, not the copy
in the library checkout).

```bash
snakemake --configfile config/config.walloon.yaml --cores 1 \
  results/walloon/scen_demande_haute/html/pypsa/index.html
```
...
fail the workflow. After the root script has been run, the same file holds the
HTTPS URL. `./cluster/nic5.sh postprocess` asks for that sentinel (set
`HTML_PUBLISH=0` or `PYPSA2HTML=0` to skip). Override the date stamp with
`HTML_PUBLISH_DATE=YYYYMMDD` (defaults to the same day as the S3 `UPLOAD_DATE`).
...
The workflow also writes the **TIMES side** of the story into `html/times/`:
one interactive Sankey per planning horizon, at two aggregation levels, rendered
by
[`times_pypsa.sankey_pages`](https://github.com/IntSusEnergySystems/TIMES_PyPSA)
from the same `sector.times_file` the demands are extracted from. pypsa2html
is not modified; `html/index.html` links the two reports.

```
results/walloon/<scenario>/html/
├── index.html                           ← hub (PyPSA + TIMES)
├── pypsa/                               ← pypsa2html
└── times/
    ├── index.html                       ← redirect to the table
    ├── times_sankey_index.html          ← year × level table, start here
...
did **not** cross the soft-link (grey). The hub at `html/index.html` is how
you open both; `./cluster/nic5.sh upload` still publishes the whole `html/`
tree with no change to `upload_s3.sh`.
...
│   └── pypsa2html.yaml             # pypsa2html report (regions, scenarios, output.dir)
