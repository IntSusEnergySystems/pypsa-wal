...
``parks``
    Machines grouped into parks of at least ``min_turbines`` at the fleet's
    single-linkage distance, 5 D between any two machines.
``parks+horizon``
...
``parks1+horizon``
    The reference without a minimum park size.
...
if __name__ == "__main__":
    ...
    def parks(case, dist, horizon=True, interdistance=0.0, minimum=n_min):
        hz = (Horizon(villages, D, lcfg["horizon_radius_m"], lcfg["min_free_azimuth_deg"])
              if horizon else None)
        idx, pid, refused = grow_parks(xs, ys, score, dist, link=link, n_min=minimum,
                                       horizon=hz, interdistance=interdistance,
                                       along_motorway=along_mw, tree=tree)
        chk = check_layout(xs[idx], ys[idx], pid, dist, n_min=minimum,
    ...
    for case, dist in spacings.items():
        ...
        if case == ref_case:
            ...
            runs += [("parks1+horizon", dict(horizon=True, minimum=1))]
