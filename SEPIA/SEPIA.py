    ...
    for country in ALL_COUNTRIES:
        for planning_horizon in planning_horizons:
         # if planning_horizon != 2020:
            ...
            biomass_p.index = biomass_p.index
            ...
            columns_to_sum = ['solid biomass', 'unsustainable solid biomass']
            df.loc[planning_horizon, country] = biomass_p.loc[country, columns_to_sum].sum()
...
def prepare_sepia(countries):
 ...
 for country in ALL_COUNTRIES:
    ...
    for en_code in ['elc']:
        value_elect = flows[(en_code + '_fe', 'res', 'elc')].squeeze().rename_axis(None) 
        dist_losses = flows[(en_code + '_se','per', 'dis')].squeeze().rename_axis(None)
        flows[(en_code + '_fe', 'res', 'elc')] = value_elect - dist_losses
    ...
    biogas_cc = flows['bgl_pe','gaz_se','cc'].squeeze().rename_axis(None).clip(lower=0)
    biosng_p = flows['enc_pe','gaz_se',''].squeeze().rename_axis(None).clip(lower=0)
    meth_p = flows['hyd_se','gaz_se',''].squeeze().rename_axis(None).clip(lower=0)
    biosng_cc = flows['enc_pe','gaz_se','cc'].squeeze().rename_axis(None).clip(lower=0)
    ...
    for en_code in ['gaz']:
        flows[(en_code+'_pe',en_code+'_se','')] = fec_se[en_code+'_se']-biogas_p-biosng_p-meth_p-biogas_cc-biosng_cc
    
    ''' Adding missing values for nuclear losses'''
    nuc_eff = float(options.loc[("nuclear", "efficiency")])
    nuc_los = (1 - nuc_eff)/nuc_eff
    for en_code in ['ura']:
      value_loss_nuc_2025 = flows.loc['2020', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      ...
      flows.loc['2020', (en_code + '_pe', 'per', 'thm')] = value_loss_nuc_2025
    ...
    fec_p = grouped_fec_p
    if country == 'EU':
     for en_code in ['hdr','eon','eof','spv','pac','enc','bgl','win','wst']:
        flows[('prod',en_code+'_pe','')] = fec_p[en_code+'_pe']
    else: 
     for en_code in ['hdr','eon','eof','spv','pac','bgl','win','wst']:
        flows[('prod',en_code+'_pe','')] = fec_p[en_code+'_pe'] 
    ...
    if country != 'EU':
     ...
     bm_potentialss = df[[country]]
     bm_potentials = bm_potentialss.loc[:, country].values
    ...
    co2_intensity_gas = float(options.loc[("gas", "CO2 intensity")])
    co2_intensity_oil = float(options.loc[("oil", "CO2 intensity")])
    co2_intensity_met = float(options.loc[("methanolisation", "carbondioxide-input")])
    
    if country != 'EU':     
     for en_code in ['fgs']:
        # values_agr_emm = flows[('gaz_fe','agr', '')].squeeze().rename_axis(None) * co2_intensity_gas
        values_gas_emm = fec_p['gaz_pe'] 
        flows_co2.loc['2020', (en_code + '_ghg', 'gas_ghg', '')] = values_gas_emm['2020'] * co2_intensity_gas
    ...
    for en_code in ['pac']:
      rural_heat = flows[(en_code + '_fe','res', 'gr')].squeeze().rename_axis(None)
    ...
    if country != 'EU': 
     for en_code in ['oil']: 
      ...
      val_oil_boildc = flows_co2[('oil_ghg', 'atm', 'rc')].squeeze().rename_axis(None)
      val_oil_ind = flows_co2[('oil_ghg', 'atm', 'oi')].squeeze().rename_axis(None)
      val_naph = flows_co2[('oil_ghg', 'hvc_ghg', 'oil')].squeeze().rename_axis(None)
      val_hvc = flows_co2[('oil_ghg', 'atm', 'hvc')].squeeze().rename_axis(None)
      val_agroil = flows_co2[('oil_ghg', 'atm', 'agr')].squeeze().rename_axis(None)
      val_avoil = flows_co2[('oil_ghg', 'atm', 'avi')].squeeze().rename_axis(None)
      val_shil = flows_co2[('oil_ghg', 'atm', 'so')].squeeze().rename_axis(None)
      val_ldoil = flows_co2[('oil_ghg', 'atm', 'tra')].squeeze().rename_axis(None)
      val_fisc = flows_co2[('stm', 'oil_ghg', '')].squeeze().rename_axis(None)
      val_elcbio = flows_co2[('bm_ghg', 'oil_ghg', 'bio')].squeeze().rename_axis(None)
      val_bmliq = flows_co2[('bm_ghg', 'oil_ghg', 'bm')].squeeze().rename_axis(None)
      flows_co2[('fol_ghg',  'oil_ghg', '')] =val_oil_boil+val_oil_boildc+val_oil_ind+ val_naph+val_hvc+val_agroil+val_avoil+val_shil+val_ldoil-val_fisc-val_elcbio-val_bmliq
    ...
    if country == 'EU':
      for en_code in ['gas']:
         value_biogas = flows_co2[('blg_ghg', en_code + '_ghg', '')].squeeze().rename_axis(None)
         value_biogascc = flows_co2[('blg_ghg', en_code + '_ghg', 'cc')].squeeze().rename_axis(None)
         value_sbm = flows_co2[('bm_ghg', en_code + '_ghg', '')].squeeze().rename_axis(None)
         value_sbmcc = flows_co2[('bm_ghg', en_code + '_ghg', 'cc')].squeeze().rename_axis(None)
         value_sab = flows_co2[('stm', en_code + '_ghg', '')].squeeze().rename_axis(None)
         value_gaz_emm = tot_em[en_code+'_ghg']
         flows_co2[('fgs_ghg', en_code + '_ghg', '')] = value_gaz_emm-value_biogas-value_biogascc-value_sbm-value_sbmcc-value_sab
 ...
 def generate_results(flows, tot_results, country):
    ...
    value_bmel = flows_bk[('enc_pe', 'pet_fe', 'bio')].squeeze().rename_axis(None)
    value_toltal = ((value_biml + value_fish + value_bmel)/(value_petr + value_biml + value_fish + value_bmel))*100
