...
def create_node_chart(results, NODES, main_params, type="area", title='', xls_writer=None, unit='TWh/year', targets={}):
    ...
    if type == "area":
        ...
        # updatemenus = []
        line_width = 2
    tickvals = [2020, 2030, 2035, 2040, 2045, 2050]

    ticktext = ["2025" if v == 2020 else str(v) for v in tickvals]
    ...
    fig.update_layout(hovermode='x',height=700, width=1400,legend_title_text='',yaxis_title=unit,font=dict(
                size=18))
    if len(targets)>0:
        fig.add_scatter(y=targets['y'], x=targets['x'], mode=targets['mode'], name=targets['title'], marker_size=18, marker_color='black')
    # else:
    #     fig.update_layout(updatemenus=updatemenus)
    if type == "area": fig.add_scatter(y=results.sum(axis=1).to_list(), x=results.index.to_list(), mode='lines', name='Total',marker_size=18, line_color="black")
...
def create_sankey(flows, nodes, processes, main_params, interval_year=5, title="Sankey diagram"):
    ...
    fig = go.Figure()
    steps = []
    years = range(flows.index.min(), flows.index.max() + 1, interval_year)
    ...
    for i, year in enumerate(years):
        # Adding nodes, which are exceptionnaly available on this year
        new_not_empty_nodes = nodes_with_links.loc[nodes_with_links[year] > 0].index.drop_duplicates()
...
# Chloropleth maps
def create_map(results, countries, title, main_params, xls_writer=None, interval_year=5, unit='%', min_scale=np.nan, mid_scale=np.nan, max_scale=np.nan, reverse=False):
    df = results.round(main_params['DECIMALS']).T # transposing results
