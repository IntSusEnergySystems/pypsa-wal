...
class _AggPNomLimitsConfig(BaseModel):
    ...
    agg_nuclear: bool = Field(
        False,
        description="Aggregate together all nuclear technologies when writing the constraint (`nuclear-all` as a carrier in the `.csv` file). Default is false.",
    )
    agg_ccgt: bool = Field(
        False,
        description="Aggregate together `CCGT` and `CCGT CC` when writing the constraint (`CCGT-all` as a carrier in the `.csv` file). Default is false.",
    )
    file: str = Field(
    ...
    growth_multiplier: float | None = Field(
        None,
        description="Bound new renewable build per planning period at this multiple of the best annual capacity addition observed in IRENA statistics, times the length of the period. Requires `build_rates_file`. Null disables the limit.",
    )
    build_rates_file: str | None = Field(
        None,
        description="Reference to the `.csv` file of observed annual capacity additions per node and carrier, generated by `scripts/walloon_scripts/build_res_build_rates.py`. Required when `growth_multiplier` is set.",
    )
