...
| **CO₂ storage** | `STORAGEMINELC` + `STORAGEMININD` store ~7.1 Mt in 2050 (Wallonia-only model). | Geology ramp + `BarHomogeneous`. Belgian `e_nom_max` **0** (no demonstrated CO2StoP site; TIMES 7.1 is not geology). Export via `co2_network` to DE/NL/GB. [co2-sequestration-20260829.md](co2-sequestration-20260829.md). | Not transferred. |
